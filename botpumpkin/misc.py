...
class Misc(commands.Cog):
    ...
    async def slap(self, context: commands.Context, member: discord.Member) -> None:
        ...
        if random.randint(1, slap_random_chance) == slap_random_chance:
            ...
            await message_util.send_simple_embed(context, f"{self.bot.user.mention} slapped {random_member.mention} instead!")
        else:
            await message_util.send_simple_embed(context, f"{self.bot.user.mention} slapped {member.mention}!")
    ...
    async def slap_error(self, context: commands.Context, exception: commands.CommandError) -> None:
        ...
        if isinstance(exception, commands.BadArgument):
            await message_util.send_simple_embed(context, f"{self.bot.user.mention} didn't know who to slap, so {context.author.mention} was slapped instead!")
        elif isinstance(exception, commands.MissingRequiredArgument):
            await message_util.send_simple_embed(context, f"{self.bot.user.mention} slapped {context.author.mention}!")
        else:
